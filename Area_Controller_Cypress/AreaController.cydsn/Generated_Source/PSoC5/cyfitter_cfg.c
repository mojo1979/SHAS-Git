...
#define BS_UDB_1_0_1_CONFIG_VAL ((const uint8 CYFAR *)0x48000400u)

/* IOPINS0_0 Address: CYREG_PRT0_DM0 Size (bytes): 8 */
#define BS_IOPINS0_0_VAL ((const uint8 CYFAR *)0x48000480u)
...
#define BS_IOPINS0_3_VAL ((const uint8 CYFAR *)0x48000498u)

/* CYDEV_CLKDIST_ACFG0_CFG0 Address: CYREG_CLKDIST_ACFG0_CFG0 Size (bytes): 4 */
#define BS_CYDEV_CLKDIST_ACFG0_CFG0_VAL ((const uint8 CYFAR *)0x480004A4u)
...
static uint8 CYXDATA * const CYCODE A_SENS__outerAddr[3] = {
	(uint8 CYXDATA *)CYREG_PRT3_AG, 
	(uint8 CYXDATA *)CYREG_PRT3_AG, 
	(uint8 CYXDATA *)CYREG_PRT3_AG, 
};

static const uint8 CYCODE A_SENS__outerMask[3] = {
	0x01u, 
	0x02u, 
	0x04u, 
};

static uint8 CYXDATA * const CYCODE A_SENS__innerAddr[2] = {
	(uint8 CYXDATA *)CYREG_DSM0_SW0, 
	(uint8 CYXDATA *)CYREG_DSM0_SW0, 
};

static const uint8 CYCODE A_SENS__innerMask[2] = {
	0x20u, 
	0x40u, 
};

static const uint8 CYCODE A_SENS__innerMap[3] = {
	0u, 
	0u, 
	1u, 
};

static uint8 A_SENS__armsEnabled = 0u;
static uint8 A_SENS__refCnt[] = {0u, 0u};

/*******************************************************************************
* Function Name: A_SENS_Set
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void A_SENS_Set(uint8 channel)
{
	uint8 i;
	uint8 channelMask = (1UL << channel);
	if ((channel < 3u) && ((A_SENS__armsEnabled & channelMask) == ((uint32)0u)))
	{
		A_SENS__armsEnabled |= channelMask;
		*A_SENS__outerAddr[channel] |= A_SENS__outerMask[channel];
		i = A_SENS__innerMap[channel];
		if (A_SENS__refCnt[i] == 0u)
		{
			*A_SENS__innerAddr[i] |= A_SENS__innerMask[i];
		}
		A_SENS__refCnt[i]++;
	}
}
...
*
*******************************************************************************/
void A_SENS_Unset(uint8 channel)
{
	uint8 i;
	uint8 channelMask = (1UL << channel);
	if ((channel < 3u) && ((A_SENS__armsEnabled & channelMask) != ((uint32)0u)))
	{
		A_SENS__armsEnabled &= (uint8)~channelMask;
		*A_SENS__outerAddr[channel] &= (uint8)~A_SENS__outerMask[channel];
		i = A_SENS__innerMap[channel];
		A_SENS__refCnt[i]--;
		if (A_SENS__refCnt[i] == 0)
		{
			*A_SENS__innerAddr[i] &= (uint8)~A_SENS__innerMask[i];
		}
	}
}
