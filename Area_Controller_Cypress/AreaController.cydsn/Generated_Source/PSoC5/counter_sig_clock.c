...
/*******************************************************************************
* Function Name: counter_sig_clock_Start
********************************************************************************
*
* Summary:
...
void counter_sig_clock_Start(void) 
{
    ...
    counter_sig_clock_CLKEN |= counter_sig_clock_CLKEN_MASK;
	counter_sig_clock_CLKSTBY |= counter_sig_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: counter_sig_clock_Stop
********************************************************************************
*
* Summary:
...
void counter_sig_clock_Stop(void) 
{
    ...
    counter_sig_clock_CLKEN &= (uint8)(~counter_sig_clock_CLKEN_MASK);
	counter_sig_clock_CLKSTBY &= (uint8)(~counter_sig_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: counter_sig_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((counter_sig_clock_CLKEN & counter_sig_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = counter_sig_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = counter_sig_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* counter_sig_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(counter_sig_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(counter_sig_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: counter_sig_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: counter_sig_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = counter_sig_clock_GetSourceRegister();
    uint16 oldDivider = counter_sig_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = counter_sig_clock_CLKEN & counter_sig_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(counter_sig_clock_DIV_PTR, clkDivider);
                counter_sig_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                counter_sig_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(counter_sig_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((counter_sig_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(counter_sig_clock_DIV_PTR, clkDivider);
				counter_sig_clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: counter_sig_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(counter_sig_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: counter_sig_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    counter_sig_clock_MOD_SRC |= modeBitMask & (uint8)counter_sig_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: counter_sig_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    counter_sig_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(counter_sig_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: counter_sig_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return counter_sig_clock_MOD_SRC & (uint8)(counter_sig_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: counter_sig_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = counter_sig_clock_GetDividerRegister();
    uint8 oldSrc = counter_sig_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        counter_sig_clock_MOD_SRC |= CYCLK_SSS;
        counter_sig_clock_MOD_SRC =
            (counter_sig_clock_MOD_SRC & (uint8)(~counter_sig_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: counter_sig_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return counter_sig_clock_MOD_SRC & counter_sig_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: counter_sig_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    counter_sig_clock_PHASE = clkPhase & counter_sig_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: counter_sig_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return counter_sig_clock_PHASE & counter_sig_clock_PHASE_MASK;
}
