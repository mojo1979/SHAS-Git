/*******************************************************************************
* File Name: LIGHT_SENS_ADC_theACLK.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: LIGHT_SENS_ADC_theACLK_Start
********************************************************************************
*
* Summary:
...
void LIGHT_SENS_ADC_theACLK_Start(void) 
{
    ...
    LIGHT_SENS_ADC_theACLK_CLKEN |= LIGHT_SENS_ADC_theACLK_CLKEN_MASK;
	LIGHT_SENS_ADC_theACLK_CLKSTBY |= LIGHT_SENS_ADC_theACLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: LIGHT_SENS_ADC_theACLK_Stop
********************************************************************************
*
* Summary:
...
void LIGHT_SENS_ADC_theACLK_Stop(void) 
{
    ...
    LIGHT_SENS_ADC_theACLK_CLKEN &= (uint8)(~LIGHT_SENS_ADC_theACLK_CLKEN_MASK);
	LIGHT_SENS_ADC_theACLK_CLKSTBY &= (uint8)(~LIGHT_SENS_ADC_theACLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: LIGHT_SENS_ADC_theACLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((LIGHT_SENS_ADC_theACLK_CLKEN & LIGHT_SENS_ADC_theACLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = LIGHT_SENS_ADC_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = LIGHT_SENS_ADC_theACLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* LIGHT_SENS_ADC_theACLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(LIGHT_SENS_ADC_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(LIGHT_SENS_ADC_theACLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: LIGHT_SENS_ADC_theACLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: LIGHT_SENS_ADC_theACLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = LIGHT_SENS_ADC_theACLK_GetSourceRegister();
    uint16 oldDivider = LIGHT_SENS_ADC_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = LIGHT_SENS_ADC_theACLK_CLKEN & LIGHT_SENS_ADC_theACLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(LIGHT_SENS_ADC_theACLK_DIV_PTR, clkDivider);
                LIGHT_SENS_ADC_theACLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                LIGHT_SENS_ADC_theACLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(LIGHT_SENS_ADC_theACLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((LIGHT_SENS_ADC_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(LIGHT_SENS_ADC_theACLK_DIV_PTR, clkDivider);
				LIGHT_SENS_ADC_theACLK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: LIGHT_SENS_ADC_theACLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(LIGHT_SENS_ADC_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: LIGHT_SENS_ADC_theACLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    LIGHT_SENS_ADC_theACLK_MOD_SRC |= modeBitMask & (uint8)LIGHT_SENS_ADC_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: LIGHT_SENS_ADC_theACLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    LIGHT_SENS_ADC_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(LIGHT_SENS_ADC_theACLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: LIGHT_SENS_ADC_theACLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return LIGHT_SENS_ADC_theACLK_MOD_SRC & (uint8)(LIGHT_SENS_ADC_theACLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: LIGHT_SENS_ADC_theACLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = LIGHT_SENS_ADC_theACLK_GetDividerRegister();
    uint8 oldSrc = LIGHT_SENS_ADC_theACLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        LIGHT_SENS_ADC_theACLK_MOD_SRC |= CYCLK_SSS;
        LIGHT_SENS_ADC_theACLK_MOD_SRC =
            (LIGHT_SENS_ADC_theACLK_MOD_SRC & (uint8)(~LIGHT_SENS_ADC_theACLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: LIGHT_SENS_ADC_theACLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return LIGHT_SENS_ADC_theACLK_MOD_SRC & LIGHT_SENS_ADC_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: LIGHT_SENS_ADC_theACLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    LIGHT_SENS_ADC_theACLK_PHASE = clkPhase & LIGHT_SENS_ADC_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: LIGHT_SENS_ADC_theACLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return LIGHT_SENS_ADC_theACLK_PHASE & LIGHT_SENS_ADC_theACLK_PHASE_MASK;
}
